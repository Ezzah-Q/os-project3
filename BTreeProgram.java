    ...
    static class BTree {
        ...
        void insert(RandomAccessFile file, long key, long value) throws IOException {
            ...
            if (rootID == 0) {
                ...
                // insert key value pair into the node's array
                ...
                // We need to see how many keys the root has
                // get root info by returning an instance of node class as root
                ...
                if (root.numKeys >= 19) {
                    ...
                    splitRoot(file, newRoot, 0);
                        // promote the median key from old full root
                        // place median key in new root
                        // old full root is split into 2 nodes
                            // one node has all keys left to median key
                                // left child of new root
                            // other node has all keys right to median key
        ...
        void insertNormally(RandomAccessFile file, BTreeNode newRoot, long key, long value) {
            // two way to insert a key
            // if the node to insert key is a leaf node
                // just loop through the node and find the correct spot to insert key


            // if the node to insert key has children
                // need to find correct child
                // see if child is full
                    // if it is split it --> splitRoot
                // if not just insert the key (recursive call back to insertNormally)
        }
